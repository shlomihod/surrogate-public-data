...
from ydnpd.harness.synthesis.src.qm import KWayMarginalQM
from ydnpd.harness.synthesis.src.algo.base import IterativeAlgorithmTorch
from ydnpd.harness.synthesis.src.utils.mechanisms import exponential_mech, gaussian_mech
...
class IterAlgoGEMBase(IterativeAlgorithmTorch):
    def __init__(self, G, T, eps0,
                 ...
                 save_interval=20, save_num=None,
                 ):
    ...
    def _optimize_past_queries(self):
        ...
        for step in range(self.max_iters):
            ...
            idxs = torch.arange(len(errors), device=device)
...
class IterAlgoGEM(IterAlgoGEMBase):
    def _sample(self, scores):
        scores[self.past_query_idxs] = -np.inf
